...
params <- expand.grid(N=c(10, 50, 100, 1000),
...
converged <- circular_dichroism_spectrum(cluster=cl, material=gold, averaging="QMC", N=5000)

p <- 
  ggplot(subset(comparison, type == "CD" & variable == "extinction"),
         aes(wavelength, value)) + 
  facet_grid(averaging~., scales="free")+
  geom_path(aes(colour=factor(N), group=N))+
